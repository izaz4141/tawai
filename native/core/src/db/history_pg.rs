...
pub async fn record_playback(
    ...
    sqlx::query(
        "INSERT INTO playback_history (id, user_id, track_id, source) VALUES ($1, $2, $3, $4)",
    )
    .bind(&id)
    .bind(user_id)
    .bind(track_id)
...
pub async fn mark_scrobbled(pool: &PgPool, history_id: &str) -> Result<()> {
    sqlx::query("UPDATE playback_history SET scrobbled = true WHERE id = $1")
        .bind(history_id)
        .execute(pool)
        .await?;
    Ok(())
...
pub async fn get_listenbrainz_token(
    ...
    let stored: Option<String> = sqlx::query_scalar::<_, String>(
        "SELECT value FROM user_settings WHERE user_id = $1 AND key = 'listenbrainz_token'",
    )
    .bind(user_id)
    .fetch_optional(pool)
    .await
